...
public class InternetDomainName
{
    ...
    private InternetDomainName(String s)
    {
    ...
    {
        return fromInternal(DOT_JOINER.join(parts.subList(i, parts.size())));
    }
...
_L5:
        ...
        if (!TldPatterns.EXACT.contains(s)) goto _L2; else goto _L1
_L1:
        return i;
_L2:
...
_L4:
    ...
    {
        s = s.split("\\.", 2);
        return s.length == 2 && TldPatterns.UNDER.contains(s[1]);
    }
    ...
    {
        return fromInternal((new StringBuilder()).append((String)Preconditions.checkNotNull(s)).append(".").append(name).toString());
    }
    ...
    InternetDomainName fromInternal(String s)
    {
        return fromLenient(s);
    }
